...
@pytest.mark.asyncio
async def test_inspect_fb_verification_status_schema(
    direct_api_mcp_client: Client[FastMCPTransport],
):
    """Inspect the schema of fb_verification_status tool."""
    tools = await direct_api_mcp_client.list_tools()

    target_tool = next(
        (t for t in tools if t.name == "fb_verification_status"), None
    )

    assert target_tool is not None, "Tool not found"

    print("\n" + "=" * 80)
    print("TOOL SCHEMA FOR: fb_verification_status")
    print("=" * 80)
...
@pytest.mark.asyncio
async def test_fb_verification_status(
    user_id: str,
    ...
    """
    print(f"\n{'=' * 80}")
    print(f"Testing fb_verification_status")
...
@pytest.mark.asyncio
async def test_fb_verification_status_missing_user_id(
    direct_api_mcp_client: Client[FastMCPTransport],
):
    ...
    with pytest.raises(Exception):
        await direct_api_mcp_client.call_tool(
            "fb_verification_status",
            arguments={},  # Missing user_id
        )


@pytest.mark.asyncio
async def test_fb_verification_status_invalid_user(
    direct_api_mcp_client: Client[FastMCPTransport],
):
    """Test that tool returns error for user with no JWT token in TempMemory."""
    result = await direct_api_mcp_client.call_tool(
        "fb_verification_status",
        arguments={"user_id": "nonexistent_user_xyz"},
    )
