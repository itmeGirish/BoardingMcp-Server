...
MAX_ITERATIONS = 20  # Content creation may need more iterations (list, submit, poll, select)
...
async def call_model_node(
    ...
    if tool_message_count >= MAX_ITERATIONS:
        logger.warning(f"[ContentCreation] Max iterations ({MAX_ITERATIONS}) reached. Forcing END.")
        return Command(goto=END, update={"messages": [AIMessage(content="Content creation complete. Maximum iteration limit reached.")]})

    recent_messages = state["messages"][-2:] if len(state["messages"]) > 2 else state["messages"]
    logger.info(f"[ContentCreation] Call model with {len(state['messages'])} messages, {tool_message_count} tool results, last 2:")
