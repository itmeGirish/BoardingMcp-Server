...
Pure node functions for the segmentation sub-graph.
...
async def call_model_node(
    ...
    """
    Call the language model node for segmentation agent.

    Args:
        state: Current agent state (SegmentationAgentState)
        config: Runnable configuration
        system_prompt: Segmentation system prompt
    ...
    try:
        ...
        logger.info("[Segmentation] Model response received successfully")
    except Exception as e:
        logger.error(f"[Segmentation] Model invocation failed: {e}", exc_info=True)
        raise
    ...
    if tool_calls:
        logger.info(f"[Segmentation] Tool calls: {[tc.get('name') for tc in tool_calls]}")
    ...
    if has_backend_tool_calls:
        logger.info("[Segmentation] Routing to tool_node (backend tools)")
        return Command(
            goto="tool_node",
            update={"messages": [response]},
        )
    else:
        logger.info("[Segmentation] Routing to END")
        return Command(
            goto=END,
            update={"messages": [response]},
        )
