...
Pure node functions for the analytics sub-graph.
Same pattern as other broadcast sub-agents.
"""
...
async def call_model_node(
    ...
    """
    Call the language model node for analytics agent.

    Args:
        state: Current agent state (AnalyticsAgentState)
        config: Runnable configuration
        system_prompt: Analytics system prompt
    ...
    if tool_message_count >= MAX_ITERATIONS:
        logger.warning(f"[Analytics] Max iterations ({MAX_ITERATIONS}) reached. Forcing END.")
        return Command(goto=END, update={"messages": [AIMessage(content="Analytics complete. Maximum iteration limit reached.")]})

    recent_messages = state["messages"][-2:] if len(state["messages"]) > 2 else state["messages"]
    logger.info(f"[Analytics] Call model with {len(state['messages'])} messages, {tool_message_count} tool results, last 2:")
    ...
    try:
        ...
        logger.info("[Analytics] Model response received successfully")
    except Exception as e:
        logger.error(f"[Analytics] Model invocation failed: {e}", exc_info=True)
        raise
    ...
    if tool_calls:
        logger.info(f"[Analytics] Tool calls: {[tc.get('name') for tc in tool_calls]}")
    ...
    if has_backend_tool_calls:
        logger.info("[Analytics] Routing to tool_node (backend tools)")
        return Command(
            goto="tool_node",
            update={"messages": [response]},
        )
    else:
        logger.info("[Analytics] Routing to END")
        return Command(
            goto=END,
            update={"messages": [response]},
        )
