...
Pure node functions for the delivery sub-graph.
...
async def call_model_node(
    ...
    """
    Call the language model node for delivery agent.

    Args:
        state: Current agent state (DeliveryAgentState)
        config: Runnable configuration
        system_prompt: Delivery system prompt
    ...
    if tool_message_count >= MAX_ITERATIONS:
        logger.warning(f"[Delivery] Max iterations ({MAX_ITERATIONS}) reached. Forcing END.")
        return Command(goto=END, update={"messages": [AIMessage(content="Delivery complete. Maximum iteration limit reached.")]})

    recent_messages = state["messages"][-2:] if len(state["messages"]) > 2 else state["messages"]
    logger.info(f"[Delivery] Call model with {len(state['messages'])} messages, {tool_message_count} tool results, last 2:")
    ...
    try:
        ...
        logger.info("[Delivery] Model response received successfully")
    except Exception as e:
        logger.error(f"[Delivery] Model invocation failed: {e}", exc_info=True)
        raise
    ...
    if tool_calls:
        logger.info(f"[Delivery] Tool calls: {[tc.get('name') for tc in tool_calls]}")
    ...
    if has_backend_tool_calls:
        logger.info("[Delivery] Routing to tool_node (backend tools)")
        return Command(
            goto="tool_node",
            update={"messages": [response]},
        )
    else:
        logger.info("[Delivery] Routing to END")
        return Command(
            goto=END,
            update={"messages": [response]},
        )
